...
from dataset.smr.recover_smr import *
from dataset.utils.rotation_conversions import rotation_6d_to_matrix,  matrix_to_rotation_6d
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        speed_loss_scale=0.0
    ):
        self.model_mean_type = model_mean_type
        self.model_var_type = model_var_type
        self.loss_type = loss_type
        self.rescale_timesteps = rescale_timesteps
        self.speed_loss_scale = speed_loss_scale
    ...
    def masked_l2(self, a, b, mask):
        ...
        loss = sum_flat(loss * mask.float())  # gives \sigma_euclidean over unmasked elements   ### [Batch]
    ...
    def mask_translation_loss(self, a, b, mask):
        transa = a[:, :67, :, :]
        transb = b[:, :67, :, :]

        bs, njoint, nfeat, nframe = transa.shape

        loss = self.l2_loss(transa, transb)              #### [bs, 263, 1, num_frames]
        loss = sum_flat(loss * mask.float())  # gives \sigma_euclidean over unmasked elements   ### [Batch]

        n_entries = njoint * nfeat     ##### BS * 263 * 1 * num_frame -> 263
        non_zero_elements = sum_flat(mask) * n_entries 
        mse_loss_val = loss / non_zero_elements
        return mse_loss_val
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, **kargs):
        """
        ...
        #################### 模型推理和 MSE 损失， mask 超出实际长度的部分不加噪声 ###############
        ...
        '''
        速度损失
        mask 先取真实长度 - 1 的位置置0，然后去掉最后一帧
        joints 速度直接算除了 translation 以外的部分，两帧之差
        rotation 速度是上一帧的转置 @ 下一帧的旋转，得到 上一帧到下一帧的速度，然后拉回 rot6D 计算差值
        '''
        if self.speed_loss_scale != 0:
            speed_mask = mask.detach().clone() ### [bs, 1, 1, nframes - 1]
            real_length = sum_flat(mask).long() - 1    #### [bs]
            speed_mask[torch.arange(speed_mask.shape[0]), :, :, real_length] = 0
            speed_mask = speed_mask[:, :, :, :-1]

            output = model_output[:, :, 0, :].permute(0, 2, 1)  ### [bs, nframes, length]
            gt = x_start[:, :, 0, :].permute(0, 2, 1)
            ################# 根节点轨迹速度 ###########
            root_pred = output[:, :, 0:3]
            root_gt = output[:, :, 0:3]
            root_speed_pred = root_pred[:, 1:, :] - root_pred[:, :-1, :]
            root_speed_gt = root_gt[:, 1:, :] - root_gt[:, :-1, :]
            root_speed_pred = root_speed_pred.permute(0, 2, 1).unsqueeze(2)
            root_speed_gt = root_speed_gt.permute(0, 2, 1).unsqueeze(2) ### [bs, 3, 1, nframes-1]
            loss_root = self.masked_l2(root_speed_pred, root_speed_gt, speed_mask)     

            ################# 21个关节点位移速度 ############
            relative_joints_infer = output[:, :, 3:66]
            relative_joints_gt = gt[:, :, 3:66]
            relative_joints_speed_infer = relative_joints_infer[:, 1:, :] - relative_joints_infer[:, :-1, :]    ### [bs, nframes-1, 63]
            relative_joints_speed_gt = relative_joints_gt[:, 1:, :] - relative_joints_gt[:, :-1, :]    ### [bs, nframes-1, 63]
            relative_joints_speed_infer = relative_joints_speed_infer.permute(0, 2, 1).unsqueeze(2)
            relative_joints_speed_gt = relative_joints_speed_gt.permute(0, 2, 1).unsqueeze(2) ### [bs, 63, 1, nframes-1]
            loss_joints = self.masked_l2(relative_joints_speed_infer, relative_joints_speed_gt, speed_mask)

            ################ 22 个关节点旋转速度 ###########
            rotation_infer = rotation_6d_to_matrix(output[:, :, 66:].reshape(output.shape[0], output.shape[1], -1, 6))   ### [bs, nframes, 22, 3, 3]
            rotation_gt = rotation_6d_to_matrix(gt[:, :, 66:].reshape(gt.shape[0], gt.shape[1], -1, 6)) ### [bs, nframes, 22, 3, 3]
            rotation_speed_infer = torch.einsum("ijklm, ijkmn -> ijkln", rotation_infer[:, :-1, :, :, :].permute(0, 1, 2, 4, 3), rotation_infer[:, 1:, :, :, :])
            rotation_speed_gt = torch.einsum("ijklm, ijkmn -> ijkln", rotation_gt[:, :-1, :, :, :].permute(0, 1, 2, 4, 3), rotation_gt[:, 1:, :, :, :]) ### [bs, nframes-1, 22, 3, 3]
            rotation_speed_infer = matrix_to_rotation_6d(rotation_speed_infer).reshape(rotation_speed_infer.shape[0], rotation_speed_infer.shape[1], -1).permute(0, 2, 1).unsqueeze(2)
            rotation_speed_gt = matrix_to_rotation_6d(rotation_speed_gt).reshape(rotation_speed_gt.shape[0], rotation_speed_gt.shape[1], -1).permute(0, 2, 1).unsqueeze(2)
            loss_rotation = self.masked_l2(rotation_speed_infer, rotation_speed_gt, speed_mask)
            
            ################## 速度损失 ###############
            loss_speed = loss_joints + loss_rotation + loss_root
        else:
            loss_speed = 0
        ...
        最终损失
        '''
        terms["loss"] = self.masked_l2(target, model_output, mask) + loss_speed * self.speed_loss_scale
