...
#        jmatch = re.match("\s*JOINT\s+(\w+)", line)
        ...
        if dmatch:
            ...
            if channel_lst.count(6) > 1 and channel_lst.count(6) < len(channel_lst):
                start_idx = 0
                end_idx = 0
                ...
                for chn_idx, chn_num in enumerate(channel_lst):
                    if chn_num == 3:
                        end_idx = start_idx + 3

                        pos = np.zeros(3)
                        rot = data_block[start_idx:end_idx]
                        ...
                        end_idx_p = start_idx + 3
                        end_idx_r = start_idx + 6

                        pos = data_block[start_idx:end_idx_p]
                        rot = data_block[end_idx_p:end_idx_r]
                ...
                positions_frm = np.stack(pos_lst, axis=0)
                rotations_frm = np.stack(rot_lst, axis=0)

                positions[fi,0:1] = positions_frm[0]
                rotations[fi, : ] = rotations_frm
...
def save(filename, anim, names=None, frametime=1.0/24.0, order='zyx', positions=False, orients=True):
    ...
    with open(filename, 'w') as f:
        ...
        rots = np.degrees(anim.rotations.euler(order=order[::-1]))
        #rots = np.degrees(anim.rotations[:,:,:])
        poss = anim.positions
