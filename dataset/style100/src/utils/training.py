...
class LossManager(object):
    ...
    def add_loss(self, iter=None,layer_id=None, **kwargs):
        if iter:
            self.iter = iter + 1
        else:
            self.iter += 1
        if layer_id is None:
            ...
            for name, loss in kwargs.items():
                if isinstance(loss, torch.Tensor):
                    loss = loss.detach().cpu().item()
                self.latest_loss[f'l{layer_id}_{name}'] = loss
                self.accum_loss[f'l{layer_id}_{name}'] += loss

    def add_loss_glb_safe(self,iter=None,**kwargs):
        if iter:
            self.iter = iter + 1
        else:
            self.iter += 1
    ...
    def print_latest(self,layer_id=None):
        info = f"Epoch {self.epoch}, iter {self.iter-1}.\t"
        if layer_id is not None:
            info = f'Layer {layer_id}, {info}'
        for name in self.loss_names:
