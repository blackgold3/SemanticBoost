...
from src.utils.o3d_wrapper import MeshO3d
...
def get_tpl_edges(remesh_obj_v, remesh_obj_f):
    '''
        给定Mesh的VF表达，生成构成边的arr：E*2
    '''
    edge_index = []
    for v in range(len(remesh_obj_v)):
        face_ids = np.argwhere(remesh_obj_f == v)[:, 0] # 找到包含当前v的face id
        neighbor_ids = []
        for face_id in face_ids:
            for v_id in range(3):
                if remesh_obj_f[face_id, v_id] != v:
                    neighbor_ids.append(remesh_obj_f[face_id, v_id]) # 当前face中非v的顶点都与v相连
        neighbor_ids = list(set(neighbor_ids)) # 当前v所连接的所有face中的邻接节点
        neighbor_ids = [np.array([v, n])[np.newaxis, :] for n in neighbor_ids] # 转化为从v出发的所有边
        neighbor_ids = np.concatenate(neighbor_ids, axis=0) # 生成arr：E(v)*2
        edge_index.append(neighbor_ids)
    edge_index = np.concatenate(edge_index, axis=0)
    return edge_index # 返回所有边的arr：E*2

def get_tpl_edges_mod(remesh_obj_v_num, remesh_obj_f):
    '''
        给定Mesh的VF表达，生成构成边的arr：E*2
    '''
    edge_index = []
    for v in range(remesh_obj_v_num):
        face_ids = np.argwhere(remesh_obj_f == v)[:, 0] # 找到包含当前v的face id
        neighbor_ids = []
        for face_id in face_ids:
            for v_id in range(3):
...
def get_nearest_face(v0, v, f):
    tri_verts = v[f.reshape(-1)].reshape([-1, 3, 3]) # [F,V,3]
    tri_centroid = np.mean(tri_verts, 1) # [F,3]
    dist = np.sum(np.square(v0[:, None] - tri_centroid[None]), 2) # [V0,1,3] - [1,F,3]
...
def get_normal(v, f):
    '''
        给定Mesh的VF表示，求顶点的法向量列表，shape=N*3
    '''
    f = f.T.astype(int)
    vec1 = v[f[1]] - v[f[0]] # 每个triangle上的一条边（从点0指向点1）,shape = F*3
    vec2 = v[f[2]] - v[f[0]] # 每个triangle上的一条边（从点0指向点2）
    face_norm = F.normalize(vec1.cross(vec2), p=2, dim=-1)  # 输入每个面的归一化法向量列表，[F, 3]

    idx = np.concatenate([f[0], f[1], f[2]], axis=0) #
    idx = torch.from_numpy(idx).long().to(v.device)
    face_norm = face_norm.repeat(3, 1) # 将法向量扩展3倍数量，和每个面的三个顶点对齐

    norm = scatter_add(face_norm, idx, dim=0, dim_size=v.size(0)) # 输出每个顶点上由其构成是所有面所决定的该点的法向量方向
    norm = F.normalize(norm, p=2, dim=-1)  # [N, 3]，顶点法向量归一化，只用于表示方向
    return norm
