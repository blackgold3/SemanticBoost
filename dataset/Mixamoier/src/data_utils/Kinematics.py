...
class ForwardKinematics:
    ...
    def transform_from_axis(euler, axis):
        transform = torch.empty(euler.shape[0:3] + (3, 3), device=euler.device) # 实际传入的euler.shape[0:3]就是高维上的批量大小，低维接一个3*3的旋转矩阵
        cos = torch.cos(euler) # 与当前轴的夹角余弦
        sin = torch.sin(euler) # 夹角正弦
        cord = ord(axis) - ord('x') # 当前轴下标，[x,y,z] = [0,1,2]

        transform[..., cord, :] = transform[..., :, cord] = 0 # 初始化旋转矩阵
        transform[..., cord, cord] = 1

        if axis == 'x': # 填写沿着当前轴旋转时，映射矩阵的解
