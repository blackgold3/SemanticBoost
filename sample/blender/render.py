...
import sample.blender.launch.blender
import sample.blender.launch.prepare  # noqa
from sample.blender.config import parse_args
...
def extend_paths(path, keyids, *, onesample=True, number_of_samples=1):
    if not onesample:
        ...
        paths = [
            template_path.replace("INDEX", str(index)) for index in range(number_of_samples)
        ]
    else:
...
def render_cli() -> None:
    ...
    keep_path = cfg.RENDER.KEEP_PATH
    ...
    from sample.blender.render_blender.blender import render
    from sample.blender.render_blender.blender.tools import mesh_detect
    from sample.blender.render_blender.video import Video
    init = True
    for path in paths:
