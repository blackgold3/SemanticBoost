...
def prune_begin_end(data, perc):
    final_length = data.shape[0] * perc
    interval = int(data.shape[0] // final_length) + 1
...
def render(npydata, frames_folder, *, mode, faces_path, gt=False,
           ...
           accelerator='gpu',device=[0], keep_path=None):
    if init:
    ...
    for index, frameidx in enumerate(frameidx):
        if mode == "sequence":
            ...
            mat = data.get_sequence_mat(frac, frameidx=frameidx)
        else:
            mat = data.get_video_mat(frameidx=frameidx)
