...
class Meshes:
    def __init__(self, data, *, gt, mode, faces_path, canonicalize, always_on_floor, oldrender=True, **kwargs):
        ...
        self.keep_path = kwargs.get("keep_path", None)
        try:
            self.keep_path = np.load(self.keep_path)
        except:
            self.keep_path = None
        ...
        self.another_mat = None
        if gt:
            self.mat = GT_SMPL
            if self.keep_path is not None:
                self.another_mat = GEN_SMPL
        else:
            self.mat = GEN_SMPL
            if self.keep_path is not None:
                self.another_mat = GT_SMPL

    def get_sequence_mat(self, frac, **kargs):
        import matplotlib

        frameidx = kargs.get("frameidx", None)
        if frameidx is not None and self.keep_path is not None and frameidx in self.keep_path:
    ...
    def get_video_mat(self, **kargs):
        frameidx = kargs.get("frameidx", None)
        if frameidx is not None and self.keep_path is not None and frameidx in self.keep_path:
            return self.another_mat
        else:
            return self.mat
