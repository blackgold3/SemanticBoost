...
class ClassifierFreeSampleModel(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if cond_mode == "nocond":       ###### 纯粹的无条件，不需要跑两次模型
            out = self.model(x, timesteps, y)
        else:       ###### CFG 跑两次模型，一次有条件，一次无条件
            y_uncond = deepcopy(y)
            ...
            output["output"] = out_uncond["output"] + (y['scale'].unsqueeze(1).unsqueeze(2).unsqueeze(3) * (out["output"] - out_uncond["output"]))
